from shoppingsimulator.simulator import LossSimulation
import pstats
import cProfile
import pandas as pd

class LossSimulationProfile:
    def __init__(self, simulation: LossSimulation, total_days: int, stockout_threshold: float):
    ...
    def get_stats(self, filename: str) -> pd.DataFrame:
        ...
        df['own_function'] = df['filename'].apply(lambda x: True if 'shoppingsimulator' in x else False)
